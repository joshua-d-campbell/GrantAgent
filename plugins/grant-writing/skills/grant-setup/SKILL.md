...
- Checkpoint policy (filename mode): milestone | per-session | per-edit   # milestone = new v<NN> only on status change, before sharing, before a structural change, or on request; working copy edited in place
