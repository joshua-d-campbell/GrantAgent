...
Read `_agent/project-config.md` first (run `grant-setup` if missing). Files: the review report goes to `_agent/reports/`; a generative-model sketch, plate diagram, or notation note written while reviewing goes to `_agent/notes/`; a plate diagram or concept figure that will be *inserted* into the Approach is a figure and goes to `02_research_plan/figures/` with its `.tex`/`.svg` source in `figures/src/` (convention 12). Identifiability or simulation checks scripted during the review go to `02_research_plan/preliminary_data/<aim>/scripts/`. Division of labor: `grant-approach` owns section structure and cross-aim logic; this skill stress-tests one component's mathematical substance; `grant-math-notation` audits form (symbols, typography, LaTeX) later — do not duplicate its passes here. Findings go to `_agent/reports/math-review_<aim>-<component>_v<NN>_<YYYY-MM-DD>.md`; approved revisions flow back into the aim's Approach file per the versioning schema. Register any symbol introduced or changed during this review in `_agent/notes/notation.md` so `grant-math-notation` starts warm.
...
Where a fix changes scope or resources, note downstream effects on `grant-budget-justification`, `grant-timeline-milestones`, and the aims page — logged as `ripple` items in `_agent/tracker.md`, one per affected document, rather than discussed now — and record material changes in `_agent/decision-log.md`. Apply revisions to the Approach text only with user approval. For notation and typography, hand off to `grant-math-notation`; for the whole section, `grant-approach`; for a skeptical scoring read once the mathematics is settled, `grant-mock-review`.
