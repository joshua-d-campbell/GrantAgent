...
**Who writes what (Word with a reference manager).** The manager owns the citations; the model does not compete with it for the file. The model never writes `ADDIN` field codes and never types formatted citation text into the document — a hand-typed entry will not renumber or restyle, and a hand-built field will not round-trip through the manager. Two outputs are allowed: (1) inserting the PI's recorded **placeholder convention** (e.g., `(PMID: 12345678)`) into the text at the anchored sentence, under the write rules in `project-config.md` (re-read from disk first, fingerprint match, no `~$` lock file, rolling pre-write backup, log the write), which the user later converts in Zotero/EndNote; and (2) a **citation worklist** — anchor sentence quoted verbatim, resolved reference with PMID/DOI, the action (insert / replace / remove / fix metadata) — that the user executes in Word. Use the worklist whenever the file is open or the fingerprint has changed. Never refer to a citation by its number across an edit boundary: numbered citations renumber on every insertion, so "add the PMID to reference 23" is wrong the moment the user inserts one upstream. Anchor to the sentence and, for fielded citations, to the CSL item ID inside the field. After the user has executed a worklist, re-read the fields from `document.xml` to verify before reporting anything as done.
...
These families are seeds, not a complete list — after the regex sweep, scan flagged-adjacent text for anything else that looks like a stand-in rather than a real citation, and add the user's variants to the config for next time. Identifier-bearing placeholders are *resolvable*, not just findings: look up the PMID/DOI (PubMed esummary, Crossref), present the resolved reference for approval, and hand it to the user for insertion through the reference manager via the worklist — never by typing formatted text (Pass 2 ground rule and the who-writes-what boundary apply). Bare placeholders go on the findings list with location and the claim they anchor. The deliverable states the placeholder count explicitly, including when it is zero.
...
`_agent/reports/reference-audit_<date>.md`: uncited claims; placeholder count and locations (explicitly zero when clean); the verification classification per reference with counts; claim-support mismatches for user review; formatting corrections. Fixes are applied with user approval: placeholders and prose fixes in the working copy under the write rules; citation insertions and metadata corrections via the worklist, executed by the user in the reference manager and verified by re-reading the fields. References still classified *not found* or *claim unchecked* stay flagged in every subsequent audit until resolved — an unresolved flag near the deadline is a decision for the user, never a silent default. These, plus uncited claims and placeholders the user defers, also go into `_agent/tracker.md` (one item each, `Origin: references`), so the open flags persist across sessions by ID instead of depending on the next audit being run.
