...
description: Rigorously review a single proposed experiment in a grant's Approach section — its rationale, hypothesis, controls, variables, sample size and power, outcomes and interpretation, feasibility, and rigor. Use whenever the user wants one experiment (or one sub-aim's experimental design) evaluated, asks whether an experiment is well designed, mentions controls, confounds, replicates, power analysis, or wants to stress-test the methods a reviewer will scrutinize. This is the fine-grained companion to grant-approach; use it to audit experiments one at a time.
---
