...
Under the NIH simplified framework, experimental rigor is the substance of Factor 2 (Rigor and Feasibility); at NSF and DoD it is where methodological reviewers concentrate (verify review-framework specifics against the current FOA and agency guide before relying on them). Reviewers do not fail proposals because the idea is uninteresting nearly as often as because a key experiment cannot answer the question it is posed to answer. This skill audits one experiment at a time, in depth, so those flaws surface before a reviewer finds them.

Read `_agent/project-config.md` first (run `grant-setup` if missing). The review report goes to `_agent/reports/`; a power calculation or simulation scripted during the review goes to `02_research_plan/preliminary_data/<aim>/scripts/` with its output in `results/` (convention 12). This is the fine-grained companion to `grant-approach`: use `grant-approach` for section structure and cross-aim logic, this skill to stress-test an individual experiment. Findings go to `_agent/reports/experiment-review_<aim>-<expt>_v<NN>_<YYYY-MM-DD>.md` (versioned per the shared schema; `<expt>` is a short experiment slug so multiple reviews within one aim don't collide); approved revisions flow back into the aim's Approach file per the versioning schema.
...
Batch all missing-information questions into a single message rather than interrogating across turns; for genuinely minor gaps, proceed with a stated assumption. State back a one-paragraph structured restatement of the experiment and get the user's confirmation. Misunderstanding the design produces useless findings; the restatement is cheap insurance.
...
Structure the findings file consistently so later skills (`grant-mock-review`, resubmission) can consume it: (1) the confirmed restatement of the experiment, (2) findings grouped by audit dimension, each tagged blocking / should-fix / optional, (3) the triage below. When applying approved revisions, findings map onto `grant-approach`'s per-aim template: dimension 1 → N.1 Rationale; dimensions 2–5 → N.3 Experimental design and N.6 Rigor; dimension 6 → N.4 Expected outcomes; dimension 7 → N.5 Pitfalls & alternatives.

Close with a short triage: the blocking issues that must be resolved for the experiment to be sound, then should-fix items ranked by how much they reduce reviewer risk. Where a fix changes scope, sample size, or resources, note the downstream effects on `grant-budget-justification`, `grant-timeline-milestones`, and the aims page — logged as `ripple` items in `_agent/tracker.md`, one per affected document, rather than discussed now — and record material design changes in `_agent/decision-log.md`. Apply revisions to the Approach text only with user approval.
