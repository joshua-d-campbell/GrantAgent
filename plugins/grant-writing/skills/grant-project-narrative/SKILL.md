...
Read `_agent/project-config.md` first (run `grant-setup` if missing). Source material is the finished aims page and Research Strategy — the narrative is written near the end, after the science is settled. Files go to `06_abstracts_title/`, versioned per the shared schema. For the Project Summary/Abstract, lay abstracts, and the rest of the abstract family, use `grant-abstracts`. Apply `_agent/style-profile.md` to every sentence of document text, and before delivering drafted or revised prose check it against the profile's *Directives* list item by item, counting any punctuation the profile constrains rather than estimating (shared convention 5).
