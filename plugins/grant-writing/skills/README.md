...
- **Fingerprint every read and log every write.** A fingerprint is size, modification time, paragraph count, and for Word the package part count and citation-field count. Before writing, compare the file to the fingerprint the session holds; on mismatch, stop, tell the user what changed, and re-derive from the current file. After writing, log the post-write fingerprint in `00_admin/version-log.md` (git mode: the commit hash) so the next session detects edits made between sessions at its first read, not mid-task.
