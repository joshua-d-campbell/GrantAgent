...
Read `_agent/project-config.md` first (run `grant-setup` if missing — setup extracts the LOI/pre-application deadline, which often falls weeks before the full deadline and is the most commonly missed date). Files go to `01_aims/loi-preproposal_v<NN>_<YYYY-MM-DD>_<status>.<ext>`, versioned per the schema. Whatever the document declares to the funder gets recorded in `_agent/decision-log.md` — declared titles, personnel, and aims are commitments the full application may be checked against. Apply `_agent/style-profile.md` to every sentence of document text, and before delivering drafted or revised prose check it against the profile's *Directives* list item by item, counting any punctuation the profile constrains rather than estimating (shared convention 5).
