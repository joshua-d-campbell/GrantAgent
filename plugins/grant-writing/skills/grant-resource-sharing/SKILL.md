...
description: Draft the NIH Resource Sharing Plan(s) — chiefly the Model Organism Sharing Plan for projects that produce novel model organisms, plus related research-tool and material sharing statements and their MTA/intellectual-property handling. Use whenever the user mentions resource sharing, model organism sharing, sharing mice/strains/reagents/cell lines/plasmids, research tools, MTAs, Bayh-Dole, or a Resource Sharing attachment. For sharing of data, software, or code, use grant-data-management-plan instead — this skill covers physical and biological resources, not data.
---
...
Read `00_admin/project-config.md` and the Approach (what novel organisms or materials the aims will generate). Output to `07_compliance/resource-sharing_v<NN>_<YYYY-MM-DD>_<status>.<ext>`. Verify current requirements against the FOA and the NIH sharing-policy pages at time of use — this skill reflects the Model Organism Sharing Policy (NOT-OD-04-042) as stated on NIH's site as of its August 2025 update, but institute-specific and FOA-specific requirements can add to it.
...
- The plan is a term of award and progress on it is weighed at renewal — do not promise distribution the lab will not sustain. Route substantive IP and compliance questions to the technology-transfer and grants offices; the skill drafts and checks completeness, it does not adjudicate.
