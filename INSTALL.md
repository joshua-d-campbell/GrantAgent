...
**Two kinds of files in the folder.** The numbered folders (`01_aims/`, `02_research_plan/`, …) hold your documents; `00_admin/` holds what you exchange with the grants office (the announcement, their checklist, submission forms); `08_final_assembly/` holds only the PDFs you upload. Everything Claude writes for its own tracking — settings, style profile, to-do list, logs, working notes, review reports — lives in one folder, `_agent/`, so it never mixes with yours. Figures go in `figures/` inside the document's folder, with their sources in `figures/src/` and the analyses that produced them in `preliminary_data/`. Folders set up before this layout keep working; ask Claude to migrate one if you want it reorganized.
